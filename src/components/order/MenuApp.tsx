...
export default function MenuApp({ categories, table }: Props) {
  ...
  return (
    <div className="menu" data-sheet={sheetOpen ? "open" : "closed"}>
      <header className="menu__head">
        <p className="menu__eyebrow">Berlin · Haus de Gourmet</p>
        <h1 className="menu__title">The Menu</h1>
        {table && <p className="menu__table">Table {table}</p>}
      </header>
      ...
      <div className="menu__controls">
        ...
        {!q && (
          <nav className="menu__jump" aria-label="Jump to category">
            {categories.map((c) => (
      ...
      {noResults ? (
        ...
        <div className="menu__sections">
          {visibleCategories.map((c) => (
            <section key={c.id} id={`cat-${c.id}`} className="section">
              <h2 className="section__name">{c.name}</h2>
    ...
    <li className="row">
      <div className="row__body">
        <div className="row__head">
          ...
          {item.is_signature && <span className="row__signature">Signature</span>}
        </div>
        {item.description && <p className="row__desc">{item.description}</p>}
        <div className="row__foot">
          <span className="row__price">{formatINR(item.price)}</span>
          {item.tags.length > 0 && (
